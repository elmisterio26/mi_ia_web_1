# -*- coding: utf-8 -*-
"""modelo IA (practicas).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Aps6ENOgzaB_CJ6HwTWMT7iVwVbHE6oT
"""

import tensorflow as tf
from sklearn.metrics import confusion_matrix, classification_report
import seaborn as sns
import matplotlib.pyplot as plt
import matplotlib.ticker as ticker
import numpy as np
import pandas as pd
import shap
# --- CONFIGURACIÓN ---
DATOS_ENTRENAMIENTO = 50000 # Prueba con 500, 5000 y 50000
NUM_EPOCHS = 6 # Vueltas al libro de datos
# ------------------------------------
(x_train, y_train), (x_test, y_test) = tf.keras.datasets.mnist.load_data()
x_train_exp = np.expand_dims(x_train / 255.0, axis=-1)
x_test_exp = np.expand_dims(x_test / 255.0, axis=-1)
# Definición del modelo (Red Neuronal Básica)
model = tf.keras.Sequential([
tf.keras.layers.Flatten(input_shape=(28, 28, 1)),
tf.keras.layers.Dense(32, activation='relu'),
tf.keras.layers.Dense(10, activation='softmax')
])
model.compile(optimizer='adam',
loss='sparse_categorical_crossentropy',
metrics=['accuracy'])



# Entrenamiento
history = model.fit(
x_train_exp[:DATOS_ENTRENAMIENTO],
y_train[:DATOS_ENTRENAMIENTO],
epochs=NUM_EPOCHS,
validation_split=0.2,
verbose=1
)

preds = np.argmax(model.predict(x_test_exp), axis=1)
plt.figure(figsize=(8,6))
sns.heatmap(confusion_matrix(y_test, preds), annot=True,
fmt='d', cmap='Blues')
plt.title("Mapa de aciertos y errores (Matriz de Confusión)")
plt.show()
print("\nREPORTE FINAL:")
print(classification_report(y_test, preds))

# Gráfica de progreso
plt.figure(figsize=(10, 4))
plt.plot(history.history['accuracy'],
label='Entrenamiento', marker='o')
plt.plot(history.history['val_accuracy'],
label='Validación', marker='s')
plt.title("¿Cómo está aprendiendo mi IA?")
plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
plt.legend()
plt.grid(alpha=0.3)
plt.show()

# Explicar un ejemplo (el primer 7 del test)
idx = 0
explainer = shap.DeepExplainer(model, x_train_exp[:100])
shap_values = explainer.shap_values(x_test_exp[idx:idx+1])
print(f"Número real: {y_test[idx]}")
shap.image_plot(shap_values, x_test_exp[idx:idx+1])

# Detectar imágenes que eran "0" pero la IA falló
idx_errores = np.where((y_test == 0) & (preds != 0))[0]
if len(idx_errores) > 0: sel = idx_errores[:10] # Ver 3 ejemplos
shap_vals_err = explainer.shap_values(x_test_exp[sel])
shap.image_plot(shap_vals_err, x_test_exp[sel])
for i, s in enumerate(sel):print(f"Ejemplo {i+1}: Real 0 | IA dijo {preds[s]}")

"""# Nueva sección"""

model.save('modelo_mnist.keras')

"""¿Por qué es necesario guardar el modelo en un archivo en
lugar de entrenarlo cada vez que alguien entra en la página web?
¿Tendría sentido?
esto se hace asi ya que entrenarla cada vez que se habra la pagina haria que la pagina tarde muchisimo en hacer las cosas ademas de que cada vez responderia con diferencias
"""